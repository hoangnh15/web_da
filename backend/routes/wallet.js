...
router.post('/change_secretkey', async (req, res) => {
    const userId = req.session.userId;
    const { password, secretkey } = req.body;
    const hashedSecretkey = await hashPassword(secretkey);
    const hashedpassword = await hashPassword(password);
    if (!password || !secretkey) {
        return res.status(400).json({code: '400' , message: 'Thiếu password hoặc secret key.' });
    }

    // Truy xuất mật khẩu đã băm từ cơ sở dữ liệu
    const selectQuery = 'SELECT pass_w FROM Register_Info WHERE ACCID = ?';
    db.query(selectQuery, [userId], async (error, results) => {
        if (error) {
            console.error('Lỗi khi thực hiện truy vấn SQL:', error);
            return res.status(500).json({code: '500' , message: 'Lỗi máy chủ nội bộ.' });
        }
        ...
        const hashedPasswordFromDB = results[0].pass_w;

        // So sánh mật khẩu người dùng nhập với mật khẩu từ cơ sở dữ liệu
        ...
        db.query(updateQuery, [hashedSecretkey, userId], (error, results) => {
            ...
            res.json({code: '200' , message: 'Secret key đã được thay đổi thành công.' });
        });
        }
        // Cập nhật secret key
