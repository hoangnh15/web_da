...
const csrfProtection = require('../middleware/csrfcheck');
const validateAndSanitizeInput = require('../utils/validate');
...
//Post lớp lên
router.post('/insertClass',csrfProtection, (req, res) => {
    ...
    console.log(validateAndSanitizeInput(detail));
    db.query(query, [classID, class_name, start_time, end_time, subID, grade, price, validateAndSanitizeInput(detail), req.session.userId], (error, results) => {
        if (error) {
            console.error('Lỗi khi thực hiện truy vấn SQL:', error);
